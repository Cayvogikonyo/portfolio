...
<manage>
        <div class="max-w-8xl mx-auto sm:px-6 lg:px-8 flex flex-col text-gray-900 dark:text-white">
            <inertia-link type="button" class="border border-green-600 text-green-600 px-4 py-1 rounded my-3 self-end bg-white" href="new/article"> <i class="fas fa-plus"></i> New Article</inertia-link>
            ...
            <div class="flex justify-center">
                ...
                <div class="w-full" id="posts-container" v-else>
                    <ul v-if="articles.length > 0" class="item-list md:grid md:grid-cols-3 md:gap-3" id="items-card">
                        <li class="w-full my-3 relative bg-gray-900 dark:bg-white bg-opacity-50" transition="expand" v-bind:style="{ backgroundImage: 'url('+ post.header +')' }" v-for="(post, index) in articles" :key="index">
                            <div class="h-40 ">
                                <div class="h-full w-full">
                                </div>
                            </div>
                            <div class="w-full shadow rounded flex z-40" >
                                <div class="px-3 p-2 w-full self-end">
                                    ...
                                    <div class="flex w-full justify-between">
                                        ...
                                        <button type="button" class="border border-blue-600 hover:bg-blue-600 bg-blue-600 text-white hover:text-white px-4 py-2 rounded my-3 editsBtn" id="more-btn" @click="edit(post.id, post.uid, index)">
                                            Edit
                                        </button>
                                        <button type="button" class="border border-red-600 hover:bg-red-600 bg-red-600 text-white hover:text-white px-4 py-2 rounded my-3 editsBtn" id="more-btn" @click="deleteItem(post.id, post.title, index)">
                                            Delete
                                        </button>
                                    </div>
                                </div>
