                        ...
                        @foreach($works as $key => $work)
                            <div class="bg-white dark:bg-gray-600 relative rounded-lg shadow my-4 transition duration-500 ease-in-out transform hover:-translate-y-1 hover:scale-105">
                                <a href="{{url('/work/'.$work->slug)}}">
                                    <img src="<?php if(!empty($work->header)){ echo $work->header; } else echo '/randoms/'.$work->randomHeader();  ?>" class="mx-auto opacity-100 h-40 rounded-lg transition duration-500 ease-in-out hover:opacity-50" alt="{{$work->header_alt}}">
                                </a>
